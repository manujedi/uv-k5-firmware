...
static void MENU_Key_UP_DOWN(bool bKeyPressed, bool bKeyHeld, int8_t Direction)
{
    ...
    if (!gIsInSubMenu) {
        ...
        //end of tab (down)
        if(Direction == -1) {
            if (Tabs[gMenuTab][0] - 1 <= gInTabSelection) {
                gMenuTab = NUMBER_AddWithWraparound(gMenuTab, +1, 0, sizeof (Tabs) / sizeof (Tabs[0]) - 1);
                gInTabSelection = Tabs[gMenuTab][0] - 1;
                gMenuCursor = Tabs[gMenuTab][gInTabSelection + 1];
                return;
            }
        } else {
        //top of tab
            if (gInTabSelection == 0) {
                gMenuTab = NUMBER_AddWithWraparound(gMenuTab, -1, 0, sizeof (Tabs) / sizeof (Tabs[0]) - 1);
                gInTabSelection = 0;
                gMenuCursor = Tabs[gMenuTab][gInTabSelection + 1];
                return;
